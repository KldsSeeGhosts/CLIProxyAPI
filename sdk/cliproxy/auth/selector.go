...
func extractSessionIDs(headers http.Header, payload []byte, metadata map[string]any) (string, string) {
	...
	// Fork: explicit OpenCode session header (kept below upstream's explicit
	// extraction so upstream-recognized identifiers keep their precedence).
	if headers != nil {
		// Scan raw keys with EqualFold: http.Header.Get canonicalizes and would
		// miss the lowercase wire spelling clients actually send.
		for name, values := range headers {
			if !strings.EqualFold(name, "x-opencode-session") {
				continue
			}
			for _, raw := range values {
				if sid := normalizedSessionCandidate(raw); sid != "" {
					return "opencode:" + sid, ""
				}
			}
		}
