...
func openAIResponsesPartFromBlock(block gjson.Result) ([]byte, bool) {
	...
	if mimeType, data, ok := openAIResponsesMediaFromBlock(block); ok {
		...
		// Fork: input_video is the CPA extension used by Pi's view_video tool;
		// keep video trimming metadata on the same Gemini part as the media bytes.
		if strings.HasPrefix(mimeType, "video/") {
			part = applyOpenAIResponsesVideoMetadata(part, block)
		}
		return part, true
	}
...
// applyOpenAIResponsesVideoMetadata copies video_metadata / videoMetadata from an
// input_video block onto the translated Gemini part.
func applyOpenAIResponsesVideoMetadata(part []byte, block gjson.Result) []byte {
