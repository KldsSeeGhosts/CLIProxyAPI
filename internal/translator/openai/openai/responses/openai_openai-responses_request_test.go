...
func TestConvertOpenAIResponsesRequestToOpenAIChatCompletions_ReusesReasoningForDeepSeekToolCallAfterAssistantMessage(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.reasoning_content").String(); got != "inspect the workspace" {
		t.Fatalf("merged assistant reasoning_content = %q, want %q; output=%s", got, "inspect the workspace", out)
	}
	if got := gjson.GetBytes(out, "messages.0.content.0.text").String(); got != "I will inspect the workspace." {
		t.Fatalf("merged assistant text = %q, want %q; output=%s", got, "I will inspect the workspace.", out)
	}
	if got := gjson.GetBytes(out, "messages.0.tool_calls.0.id").String(); got != "call_1" {
...
func TestConvertOpenAIResponsesRequestToOpenAIChatCompletions_ConvertsBuiltinShellTools(t *testing.T) {
	tests := []struct {
		name, raw, wantName string
	}{
		{name: "shell", raw: `{"model":"cursor-grok-4.6","input":"pwd","tools":[{"type":"shell"}]}`, wantName: "shell"},
		{name: "local_shell", raw: `{"model":"cursor-grok-4.6","input":"pwd","tools":[{"type":"local_shell"}]}`, wantName: "local_shell"},
		{name: "named shell", raw: `{"model":"cursor-grok-4.6","input":"pwd","tools":[{"type":"shell","name":"terminal"}]}`, wantName: "terminal"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			out := ConvertOpenAIResponsesRequestToOpenAIChatCompletions("cursor-grok-4.6", []byte(test.raw), true)
			if got := gjson.GetBytes(out, "tools.0.function.name").String(); got != test.wantName {
				t.Fatalf("function.name = %q, want %q; output=%s", got, test.wantName, out)
			}
			if got := gjson.GetBytes(out, "tools.0.function.parameters.properties.command.type").String(); got != "string" {
				t.Fatalf("command type = %q, want string; output=%s", got, out)
			}
		})
	}
}
