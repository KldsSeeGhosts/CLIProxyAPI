...
// ConversationTokenDetails is Cursor's checkpoint context-window accounting.
// used_tokens is the absolute conversation size, not a per-turn output delta.
type ConversationTokenDetails struct {
	UsedTokens int64
	MaxTokens  int64
}

// DecodeConversationTokenDetails reads ConversationStateStructure.token_details.
func DecodeConversationTokenDetails(checkpoint []byte) ConversationTokenDetails {
	raw := decodeBytesField(checkpoint, CSS_TokenDetails)
	if len(raw) == 0 {
		return ConversationTokenDetails{}
	}
	return ConversationTokenDetails{
		UsedTokens: decodeVarintField(raw, CTD_UsedTokens),
		MaxTokens:  decodeVarintField(raw, CTD_MaxTokens),
	}
}

// DecodeConversationUsedTokens returns token_details.used_tokens, or 0 if absent.
func DecodeConversationUsedTokens(checkpoint []byte) int64 {
	return DecodeConversationTokenDetails(checkpoint).UsedTokens
}
