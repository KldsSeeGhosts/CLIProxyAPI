...
// NewMemoryH2Stream returns an in-memory stream for executor tests. Write is a
// no-op; inject inbound frames with PushData.
func NewMemoryH2Stream() *H2Stream {
	s := &H2Stream{
		id:         "memory",
...
// PushData injects a Connect frame into a memory stream's inbound channel.
func (s *H2Stream) PushData(data []byte) {
	s.dataCh <- append([]byte(nil), data...)
}
